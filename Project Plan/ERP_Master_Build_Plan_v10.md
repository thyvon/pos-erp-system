...
Stock count live-entry refinement: default count sessions should support shared live counting by multiple users in the same session. Repeated scans or searches of the same product/variation must accumulate into one counted total for that session, while the system keeps an internal append-only count-entry journal for auditability and future expansion into lot/serial-specific count modes.
Stock count edit refinement: users must be able to correct a counted row total during an in-progress session. Editing a row sets a new absolute counted total in the UI, but the backend should record only the delta as a new append-only count-entry row so the live count history remains auditable.
