...
Stock transfer special rule: warehouse FROM and TO dropdowns show ALL warehouses in the business (cross-branch transfers allowed). Transfer list is filtered to show only transfers where FROM warehouse OR TO warehouse belongs to one of the user's allowed branches.
...
Stock transfer workflow refinement: transfers should not be treated as instantly completed. Creation records the document as `sent`, captures `sent_by` and `sent_at`, and posts only the outbound stock movement from the source warehouse. A separate destination-side confirmation moves the transfer to `received`, captures `received_by` and `received_at`, and posts the inbound stock movement into the destination warehouse. This provides an explicit sender/receiver agreement trail and prevents destination stock from appearing before receipt is acknowledged.
...
Stock transfer pending-state refinement: the initial transfer document should be created as `pending`, not as a posted movement. While `pending`, the source quantity is reserved/locked so it cannot be reused elsewhere, but no stock is deducted and no transfer movement audit event is posted yet. Pending transfers may be edited or deleted only by the original sender (owner) or an admin-level user. When the sender confirms `Send`, the reservation is consumed into the outbound transfer movement and the document becomes `sent`. The destination later confirms `received` and only then the inbound movement is posted.
...
Tests: Branch-scoped inventory_manager can only view and adjust stock in warehouses linked to their allowed branches. Stock transfer between two branches creates correctly and appears in both users' transfer lists.
