...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'unit_id' => [
                Rule::requiredIf(fn () => $this->input('type', $product?->type) !== 'service'),
                'nullable',
                'uuid',
            ...
            'variation_template_ids' => [
                Rule::requiredIf(fn () => $this->input('type', $product?->type) === 'variable'),
                'nullable',
                'array',
                'min:1',
            ],
            ...
            'selling_price' => ['sometimes', Rule::requiredIf(fn () => $this->input('type', $product?->type) !== 'variable'), 'nullable', 'numeric', 'min:0', 'max:9999999999.99'],
            'purchase_price' => ['sometimes', Rule::requiredIf(fn () => $this->input('type', $product?->type) !== 'variable'), 'nullable', 'numeric', 'min:0', 'max:9999999999.99'],
            ...
            'variations' => [Rule::requiredIf(fn () => $this->input('type', $product?->type) === 'variable'), 'nullable', 'array', 'min:1'],
            'variations.*.id' => [
                'nullable',
                'uuid',
            ...
            'combo_items' => [Rule::requiredIf(fn () => $this->input('type', $product?->type) === 'combo'), 'nullable', 'array', 'min:1'],
            ...
            'packagings.*.id' => [
                'nullable',
                'uuid',
                Rule::exists('product_packagings', 'id')->where(fn ($query) => $query->where('business_id', $businessId)->whereNull('deleted_at')),
            ],
