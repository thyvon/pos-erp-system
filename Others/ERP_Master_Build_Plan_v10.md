...
Sales tax refinement: the sale form must support two tax scopes: `line` and `sale`. `line` means tax follows each selected product's configured `tax_rate_id` and `tax_type` by default. `sale` means the user applies one business tax rate to the whole document after the order discount. The UI should default to `line`, expose `sale` as an explicit option, and keep manual tax editing secondary to the configured tax master data.
Ultimate-style sales workflow refinement: the sales page should follow an action-led flow similar to Ultimate POS. For normal sales, the user should not have to manage a large `document type` selector first. The main sale actions should stay familiar and mostly match the Ultimate POS mental model: `Save Draft`, `Save Quotation`, `Suspend`, `Save Sale`, `Finalize Sale`, plus the same operational list actions in most cases.
POS checkout refinement: payment entry should happen in the finalize checkout step, not as a repeated decision inside the main sale form. Users should not have to perform a separate user-facing `confirm` step. `Save Sale` reserves stock for operational sale documents. `Finalize Sale` opens the payment popup, then completes the sale and records any provided payment. Later top-up or settlement payments may still be added from the sales list/detail flow.
Sale edit lifetime refinement: sale editability should not be limited only by a hardcoded draft status. A sale may remain editable until its configured edit lifetime expires, provided the user has permission. The lifetime must come from business settings (for example `sale_edit_lifetime_days = 30`). Within that allowed lifetime, the sale can still be edited through the same sale form flow. After the lifetime expires, the document becomes locked for normal editing unless an elevated override rule is added later.
