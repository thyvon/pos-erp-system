<template>
  <Teleport to="body">
    <Transition name="erp-fade">
      <div
        v-if="show"
        class="erp-modal-overlay fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/44 px-3 py-4 backdrop-blur-2xl dark:bg-slate-950/62"
        :class="overlayClass"
        @click.self="handleBackdrop"
      >
        <div
          class="erp-modal-surface relative flex w-full flex-col overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/78 text-slate-900 shadow-[0_34px_90px_rgba(15,23,42,0.22)] backdrop-blur-[30px] dark:border-slate-700/70 dark:bg-slate-950/54 dark:text-slate-100 dark:shadow-[0_34px_90px_rgba(0,0,0,0.44)]"
          :class="[sizeClass, containerClass]"
        >
          <div class="erp-modal-tint pointer-events-none absolute inset-0 bg-gradient-to-b from-white/92 via-white/72 to-white/54 dark:from-slate-800/88 dark:via-slate-900/76 dark:to-slate-950/68"></div>
          <div class="erp-modal-glow pointer-events-none absolute inset-x-0 top-0 h-24 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.72),transparent_72%)] opacity-85 dark:bg-[radial-gradient(circle_at_top,rgba(148,197,255,0.2),transparent_72%)] dark:opacity-90"></div>
          <div class="erp-modal-inner-border pointer-events-none absolute inset-px rounded-[27px] border border-white/70 dark:border-white/5"></div>
          ...
          <div class="relative z-10 flex items-start justify-between gap-4 border-b border-slate-200/80 px-5 py-4 dark:border-white/10">
            ...
            <button
              type="button"
              class="erp-modal-close inline-flex h-10 w-10 items-center justify-center rounded-[16px] border border-slate-200/80 bg-white/78 text-slate-600 shadow-[inset_0_1px_0_rgba(255,255,255,0.82)] backdrop-blur-md transition hover:border-sky-300 hover:bg-white hover:text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:border-sky-300/30 dark:hover:bg-white/10 dark:hover:text-white"
