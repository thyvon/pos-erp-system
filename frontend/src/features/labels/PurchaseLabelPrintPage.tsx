...
export function PurchaseLabelPrintPage({ purchaseId, receiveId }: PurchaseLabelPrintPageProps) {
  ...
  const labels = useMemo(() => expandLabels(sourceItems, options), [sourceItems, options])
      ...
      <Stack className="label-print-controls" direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ alignItems: { xs: 'stretch', md: 'center' }, justifyContent: 'space-between' }}>
        <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
          <Tooltip title={t('detail.back')}>
            <IconButton size="small" component={NextLink} href={`/purchases/${purchaseId}`}>
              <ArrowBack />
            </IconButton>
          </Tooltip>
          <LocalOfferOutlined color="primary" />
          <Box>
            <Typography variant="h4">{t('labelPrinting.title')}</Typography>
              ...
              <Stack spacing={2}>
                ...
                <Alert severity={sourceItems.length === 0 ? 'warning' : 'info'}>
                  {sourceItems.length === 0
                    ...
                    : t('labelPrinting.previewCount', { count: labels.length })}
                </Alert>
              </Stack>
            </CardContent>
          </Card>
